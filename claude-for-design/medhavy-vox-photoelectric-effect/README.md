...
What if a single experiment forced physicists to abandon everything they knew about light? UV light hits metal — and out come electrons. Three things happen that waves simply cannot explain. Let's find out why.

The current plan contains **20 beats**. Its runtime is derived from 20 beats. The source recorded by the project is: See the local source and fact-check files.
...
5. **Generate local audio:** use Kokoro voice `af_kore` in the Wonder register. Measured audio durations become the master clock; never repair timing by hand.
