# QC Report — PASS

- Master: `vox-anomalous-zeeman-corrected.mp4`
- Probe: 198.96 seconds, 1920×1080, 24 fps
- Fill: 15/15 beats; no slates
- Voice: Kokoro `af_kore`; Kore explicitly in for Humanitarians AI; cost $0.00
- Visual inspection: final contact sheet read after correction pass; zero blocker and zero major defects

## Fix made during QC

- Rebuilt B05 as a wide three-column `l,s,j → Landé g_J → level slope` flow, eliminating overlap among the central box, arrows, and output formula.

## Content checks visible in the final cut

- Orbital-only regular triplet and spin magnetic moment shown.
- Weak-field criterion and `ΔE=g_J μ_B B m_J` shown.
- `2P3/2, g_J=4/3` and `2P1/2, g_J=2/3` fans shown separately.
- Corrected adjacent/total-doublet spacings `(4/3)μ_BB` and `(2/3)μ_BB` shown.
- Allowed-transition comb shown without claiming g factors predict brightness.
- Line positions separated from matrix-element/population/polarization/geometry intensities.
- Weak, intermediate, and Paschen–Back regimes distinguished.
- Your Turn values `7.72×10^-5 eV`, `3.86×10^-5 eV`, ratio 2 shown.

The native dark title outro is intentional and visibly carries the correct title plus “Kore, in for Humanitarians AI”; the automated Claude-outro warning is non-blocking after inspection.
