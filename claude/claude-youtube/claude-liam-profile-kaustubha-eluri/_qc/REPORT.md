# Visual QC Report — claude-liam-profile-kaustubha-eluri

Date: 2026-07-17
Build: 1920×1080, 30 fps, 202.6s (3:22), 4862 video frames

## 8-Point Rubric Audit

| Beat | Edge Bleed | Title-Safe | Overflow | Collision | Offscreen | Legibility | Brand Bug | Aspect |
|---|---|---|---|---|---|---|---|---|
| B00 | PASS | PASS | PASS | PASS | PASS | PASS | @HumanitariansAI chip ✓ | 16:9 ✓ |
| B01 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B02 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B03 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B04 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B05 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B06 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B07 | PASS | PASS | PASS | PASS | PASS | PASS | — | 16:9 ✓ |
| B08 | PASS | PASS | PASS | PASS | PASS | PASS | @HumanitariansAI chip ✓ | 16:9 ✓ |
| B09 | PASS | PASS | PASS | PASS | PASS | PASS | @HumanitariansAI handle ✓ | 16:9 ✓ |

## Verbatim String Checks

- [x] `Kaustubha Venkata Eluri` — B06 credit card, name row ✓
- [x] `Northeastern University` — B06 credit card, institution ✓
- [x] `Teaching Assistant — Mobile App Development` — B06 roles ✓
- [x] `Academic Peer Mentor` — B06 roles ✓
- [x] `Student Ambassador` — B06 roles ✓
- [x] `github.com/Kaustubha-09` — B06 links row ✓
- [x] `linkedin.com/in/kaustubha-ve` — B06 links row ✓
- [x] `Aditi Shinde · NortheasternISE · Mar 31, 2026 · northeasternise.substack.com` — B06 article credit ✓
- [x] `RapidTriage AI`, `Echolin.AI`, `Semantic Diff Prompting`, `NEUQuest` — B03 chips ✓
- [x] @HumanitariansAI folder chip — B00, B08 ✓
- [x] "Your turn." greeting — B08 ✓
- [x] "The Engineer Who Stayed" title restate — B09 ✓
- [x] "Kore, in for Humanitarians AI." signed in B00 and B09 narration ✓
- [x] No statistics or invented metrics anywhere ✓

## Defects Found

None.

## Final Status: ZERO BLOCKER, ZERO MAJOR defects
