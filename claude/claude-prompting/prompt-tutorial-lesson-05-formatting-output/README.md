...
# Prefill: Control Claude's Output Format

## What this video is about

**Topic:** PROMPT ENGINEERING · LESSON 5

Prefilling Claude's response to lock its output format is a trick I want to actually master, not just admire. Help me: (1) explain how starting the assistant turn with an opening brace or a tag forces structured output and suppresses preamble; (2) write a prompt-plus-prefill pair that guarantees valid JSON with three named keys; (3) show me one case where a prefill backfires — the model breaks format anyway or the p…
