...
# Why an agent with fresh memory every session still knows exactly where it left off

## What this video is about

**Topic:** Externalized state across context windows

Session 1 implements features 1–50, marks each passing in `feature_list.json`, commits. Session 2 boots with zero conversation history, reads the file, sees 51–200 still incomplete, and starts at 51 — no re-derivation.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-quickstarts/autonomous-coding/autonomous_agent_demo.py`
