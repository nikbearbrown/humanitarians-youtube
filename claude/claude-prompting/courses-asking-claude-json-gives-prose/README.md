...
# Why asking Claude for JSON gives you prose — but a tool you never call gives you clean JSON

## What this video is about

**Topic:** Two output channels, one schema

Prompt says "Return valid JSON" → Claude writes "Here is the JSON: {...}" → downstream `json.loads` throws. Define a `record_result` tool with the identical schema → Claude fills it → pure JSON arrives in the `tool_use` block → parser passes every call.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/courses/tool_use/03_structured_outputs.ipynb`
