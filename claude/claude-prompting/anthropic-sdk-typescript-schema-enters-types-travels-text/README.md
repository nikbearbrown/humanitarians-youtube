...
# Why a schema enters as types, travels as text, and returns as types

## What this video is about

**Topic:** Round-trip type safety through structured-output parsing

Pass a Zod schema to `zodOutputFormat`, call `.parse()`, read `.parsed_output` as a typed object — though the wire only ever carried a JSON Schema out and a JSON string back.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/anthropic-sdk-typescript/helpers.md`
