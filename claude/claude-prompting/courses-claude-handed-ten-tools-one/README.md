...
# Why Claude, handed ten tools for one request, sometimes picks the wrong one

## What this video is about

**Topic:** Tool selection under ambiguity

"What time is sunset?" could call `calendar_tool` or `astronomy_tool`. With near-identical descriptions Claude picks unpredictably; sharpen one ("returns exact time and refraction offset") and the request routes to the tool that truly owns the answer.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/courses/tool_use/05_tool_choice.ipynb`
