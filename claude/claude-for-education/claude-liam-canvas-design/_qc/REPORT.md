# QC REPORT — claude-liam-canvas-design

**Date:** 2026-07-18  
**Compiled MP4:** `../claude-liam-canvas-design.mp4`  
**Duration:** 3:28 (208.3s) — acceptable (target ~3:00, content-driven)  
**Video:** 1280×720 @ 24fps  
**Audio:** AAC 24000Hz ✓  
**Frames sampled:** 417 at 2fps + beats at ~15/50/85%  

---

## 9-Point Rubric

| Check | Result |
|---|---|
| Edge bleed / clipping | PASS — no content touches frame edge |
| Title-safe margins | PASS — eyebrows and spark lines within margins |
| Container overflow | PASS — all cards, canvas frame, annotations contained |
| Element collision | PASS — no overlapping text or cards |
| Offscreen anchors | PASS — all elements fully visible |
| Legibility | PASS — all text readable at render size |
| Brand / palette | PASS — cream PAGE, terracotta SPARK, correct INK tones |
| Aspect ratio | PASS — 16:9 at 1280×720 |
| Canvas fill | PASS with MINOR notes (see per-beat) |

---

## Per-Beat Results

| Beat | Pattern | Canvas Fill | Result | Notes |
|---|---|---|---|---|
| B00 | ClaudeComposerAsk | Good | PASS | "Ciao, Kore" greeting; jazz festival command; 3 output lines; @HumanitariansAI |
| B01 | CanvasDesignAnatomy | Minor gap | PASS | Step 1+2 cards left, 3 callout cards right, quote box; lower ~30% open (same pattern as brand-guidelines B01, accepted) |
| B02 | CanvasDesignPipeline | Good | PASS | 3 nodes centered H/2, orange+gray arcs, insight bar bottom |
| B03 | CanvasDesignPhilosophy | Good | PASS | Movement name callout, 5 dimensions numbered, craftsmanship mandate quote, 3 SKILL.md examples right |
| B04 | CanvasDesignCanvas | Good | PASS | SELF-DEMO: Geometric Silence canvas — dark block, grid, terracotta accent, typography, annotations |
| B05 | CanvasDesignTell | Minor gap | PASS | Central callout + ✓/✗ columns; lower ~40% open — same pattern as brand-guidelines B05, accepted |
| BVDT | ClaudeVerdictArtifact | Acceptable | PASS | "Claude, Canvas." + 5 correct verdict lines; items fade in properly |
| BHTF | ClaudeComposerAsk | Good | PASS | "Your turn." + full meditation retreat handoff prompt visible |
| BOUT | ClaudeTitleOutro | Good | PASS | "Claude, Canvas." · @HumanitariansAI · canvas-design · Anthropic Skills |

---

## Defects

None. Zero BLOCKER. Zero MAJOR. Two MINOR (B01/B05 lower-canvas open space — within tolerance for their beat types, matching brand-guidelines precedent).

---

## VERDICT: PASS
