...
**Topic:** Where a secret is allowed to live

Prototype uses `auth: .apiKey("sk-ant-...")` in source — fine locally. Before App Store release you realize the key is extractable from the binary and must move to `auth: .proxied(headers:[...])`, adding a backend relay that injects the real key server-side.
