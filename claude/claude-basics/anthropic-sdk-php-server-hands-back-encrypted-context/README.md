...
**Topic:** Streaming context compaction — readable summary vs. sealed token

A `compaction` block carries `content: "Earlier conversation summarized."` next to `encrypted_content: "EpwBCioIDxgC..."`; thread the readable text back and context breaks; thread the blob back and it holds.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/anthropic-sdk-php/fixtures/beta/compaction_encrypted_content.txt`
