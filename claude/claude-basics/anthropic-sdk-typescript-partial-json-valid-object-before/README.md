...
**Topic:** Incremental parsing of streaming tool inputs

Streaming a `search_database` call, `.on('inputJson')` fires each chunk with a `jsonSnapshot` that is already a valid JS object even though the source string has no closing brace yet.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/anthropic-sdk-typescript/src/_vendor/partial-json-parser/README.md`
