...
Merhaba — this is Kore, in for Humanitarians AI. Ask Claude to package an MCP server, bundle an MCP, make an MCPB, or ship a local MCP server — the Build MCPB skill fires. MCPB is a local MCP server packaged with its runtime: the user installs one file and it runs without needing Node, Python, or any toolchain on their machine. It is the sanctioned way to distribute local servers — and it comes with two traps that c…
