...
Salve — this is Liam, in for Bear. Ask Claude to build an MCP app, add interactive UI to an MCP server, make a form or picker that renders in chat, or use the apps SDK — the Build MCP App skill fires. An MCP app is a standard MCP server that also serves UI resources — interactive components rendered inline in the chat surface as iframes. The UI layer is additive: under the hood it's still tools and resources and the…
