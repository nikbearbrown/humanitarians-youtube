...
Merhaba — this is Kore, in for Humanitarians AI. You are working with the mcp-integration skill. It connects Claude Code plugins to external services via Model Context Protocol — four transport types, two config methods, automatic OAuth, and a tool-naming convention that must match character for character. The key decisions: which transport type for the service, dot-mcp-dot-json versus inline mcpServers, and which t…
