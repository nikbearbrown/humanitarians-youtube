...
Olá — this is Kore, in for Humanitarians AI. Ask Claude to add an MCP server, integrate MCP, configure .mcp.json, set up Model Context Protocol, or connect an external service to a plugin — the MCP Integration skill fires. MCP enables Claude Code plugins to integrate with external services by providing structured tool access. Two configuration methods: a dedicated .mcp.json at the plugin root, recommended, or an inl…
