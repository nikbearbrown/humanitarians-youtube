...
Ciao — this is Kore, in for Humanitarians AI. Ask Claude to build an MCP server, create an MCP, make an MCP integration, or wrap an API for Claude — the Build MCP Server skill fires. It is a discovery-and-routing skill: it asks four questions first, then hands off to a specialized skill like build-mcp-app or build-mcpb. Discovery-before-code is the whole model — the wrong deployment choice early means a painful rewr…
