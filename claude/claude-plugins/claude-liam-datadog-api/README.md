...
**Topic:** DATADOG API · CLAUDE TAG PLUGIN

Bonjour — this is Liam, in for Bear. Search logs, look at a metric, check which monitors are alerting, investigate a trace, pull SLO status, mute an alert, or ask what's happening in Datadog — and the Datadog API skill fires. The Datadog API is split across two stable versions that coexist — neither supersedes the other. v1 covers metrics, monitors, dashboards, SLOs, and classic events. v2 covers logs, events search…
