...
**Topic:** REDSHIFT-API · CLAUDE PLUGINS

Merhaba — this is Liam, in for Bear. You are working with the redshift-api skill. It runs SQL against Amazon Redshift — provisioned clusters and Serverless — via the Redshift Data API. Three rules govern everything. First: every call is a POST to https://redshift-data dot region dot amazonaws dot com slash, with Content-Type application x-amz-json-1.1 and X-Amz-Target RedshiftData dot action — there are no REST path…
