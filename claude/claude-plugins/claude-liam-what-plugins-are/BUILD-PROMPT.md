# BUILD-PROMPT — Claude, Equipped (`claude-liam-what-plugins-are`)
...
```
Build the deep-explainer reel at books/anthropics/books/claude-cowork-plugins/youtube/claude-liam-what-plugins-are/.
...
2. LANE-MIX LINT. Run the beat-mix histogram. VOX is at ~17% (target 20–25%). If a
   real Tier-0/Tier-1 still is cheap, promote one Act IV illustration beat to VOX;
   otherwise proceed — it clears the 15% WARN floor.
...
5. SHOPPING LIST (Gate D2, AFTER audio lock). For every VOX beat (B05, B11, B12,
   B16), first run python3 runtime/scripts/pantry_search.py on its pantry_note terms
   against svg/svg/images/. Copy real matches into pantry/ pre-checked; write
   SHOPPING.md from the LOCKED durations for the rest. All four are Tier 1
   illustrative (no rights escalation, no real people).
...
terracotta #D97757): scale_shallow_vs_deep, subagents_fan_out, divergence_general_to_specific,
threshold_stock_vs_configured, accumulate_choose_plugins. Remotion: ChipGrid,
PredictCard, SourceFlow, code-block (Onda), plus the three illustration scenes named
in the sheet. Retint deck patterns to the claude stage (cream #F2F0E9, ink #3D3929,
accent #D97757) and log the retint.
