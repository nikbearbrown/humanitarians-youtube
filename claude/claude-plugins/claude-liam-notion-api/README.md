...
Merhaba — this is Kore, in for Humanitarians AI. You are working with the notion-api skill. It integrates Claude with Notion — searching, reading, and writing pages, databases, and blocks. The key distinction: database IDs and data source IDs are not the same thing. Schema reads, queries, and row creation all take a data source ID; queries sent to the database ID return object not found. There are two bundled script…
