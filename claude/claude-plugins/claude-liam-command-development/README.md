...
Hej — this is Liam, in for Bear. Ask Claude to create a slash command, add a command, write a custom command, or get guidance on slash command structure — and the Command Development skill fires. Slash commands are Markdown files containing prompts that Claude executes during interactive sessions: define once, invoke with a slash, reuse across sessions. Let me walk you through the fundamental framing distinction, th…
