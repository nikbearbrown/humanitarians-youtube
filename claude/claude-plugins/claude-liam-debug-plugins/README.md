...
Ciao — this is Liam, in for Bear. A user asks why their plugin isn't showing up, says their skill isn't working, wants to know if their configuration took effect — the debug-plugins skill fires. It diagnoses plugin and skill loading from inside the session container: mount directories, the Claude Code launch command, and the startup log. Six steps in order, all findings collected before reporting. Let me walk you th…
