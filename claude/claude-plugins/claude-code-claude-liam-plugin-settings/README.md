...
**Topic:** PLUGIN SETTINGS · CLAUDE CODE SKILL

Shalom — this is Liam, in for Bear. Ask Claude to store plugin configuration, make plugin behavior configurable, use dot-local-dot-md files, save plugin state, or read YAML frontmatter from a settings file — the Plugin Settings skill fires. The pattern is a single file: .claude/plugin-name.local.md in the project root, with YAML frontmatter for structured config and a markdown body for prompts or context. Three cons…
