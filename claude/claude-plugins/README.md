# Claude Plugins

Videos about Code plugins, Cowork plugins, marketplace mechanics, and plugin creation. Keep the two plugin systems explicitly distinguished.

<!-- BEGIN BRUTALIST REBUILD GUIDE -->

# Claude Plugins

This folder organizes **46 video projects** built around beat sheets. Each project README explains the subject, supplies research and fact-check prompts, and documents the free local rebuild workflow.

## Rebuild toolkit

```bash
git clone https://github.com/nikbearbrown/brutalist.art.git
cd brutalist.art
./setup --install
./setup
```

Brutalist is audio-first and local: the beat sheet drives narration, measured audio becomes the clock, generated visual beats compile immediately, and unavailable media remains as labeled slates until a human fills the pantry. The human conducts, watches, fact-checks, refines, and decides whether anything is published.

## Projects in this folder

- [Claude Code Claude Liam Plugin Settings](./claude-code-claude-liam-plugin-settings/)
- [Claude Code Claude Liam Plugin Structure](./claude-code-claude-liam-plugin-structure/)
- [Access](./claude-liam-access/)
- [Agent Development](./claude-liam-agent-development/)
- [Asana Api](./claude-liam-asana-api/)
- [Bigquery Api](./claude-liam-bigquery-api/)
- [Building Plugins](./claude-liam-building-plugins/)
- [Cardputer Buddy](./claude-liam-cardputer-buddy/)
- [Claude Automation Recommender](./claude-liam-claude-automation-recommender/)
- [Claude Md Improver](./claude-liam-claude-md-improver/)
- [Combining Plugins](./claude-liam-combining-plugins/)
- [Command Development](./claude-liam-command-development/)
- [Config Guide](./claude-liam-config-guide/)
- [Configure](./claude-liam-configure/)
- [Confluence Api](./claude-liam-confluence-api/)
- [Datadog Api](./claude-liam-datadog-api/)
- [Debug Plugins](./claude-liam-debug-plugins/)
- [Enterprise Search](./claude-liam-enterprise-search/)
- [Example Command](./claude-liam-example-command/)
- [Google Drive Api](./claude-liam-google-drive-api/)
- [Grafana Api](./claude-liam-grafana-api/)
- [Graphing](./claude-liam-graphing/)
- [Hook Development](./claude-liam-hook-development/)
- [Hubspot Api](./claude-liam-hubspot-api/)
- [Installing Plugins](./claude-liam-installing-plugins/)
- [Jira Api](./claude-liam-jira-api/)
- [Linear Api](./claude-liam-linear-api/)
- [M5 Onboard](./claude-liam-m5-onboard/)
- [Notion Api](./claude-liam-notion-api/)
- [Pagerduty Api](./claude-liam-pagerduty-api/)
- [Playground](./claude-liam-playground/)
- [Plugin Settings](./claude-liam-plugin-settings/)
- [Plugin Structure](./claude-liam-plugin-structure/)
- [Project Artifact](./claude-liam-project-artifact/)
- [Redshift Api](./claude-liam-redshift-api/)
- [Salesforce Api](./claude-liam-salesforce-api/)
- [Sentry Api](./claude-liam-sentry-api/)
- [What Plugins Are](./claude-liam-what-plugins-are/)
- [Claude Plugins Community Copyright Restrictions Mean Three Compl](./claude-plugins-community-copyright-restrictions-mean-three-compl/)
- [Claude Plugins Community Image Model S Burned Captions](./claude-plugins-community-image-model-s-burned-captions/)
- [Claude Plugins Official Claude Liam Plugin Settings](./claude-plugins-official-claude-liam-plugin-settings/)
- [Claude Plugins Official Claude Liam Plugin Structure](./claude-plugins-official-claude-liam-plugin-structure/)
- [Claude Plugins Official Enabling Sms Breaks Access Control](./claude-plugins-official-enabling-sms-breaks-access-control/)
- [Claude Plugins Official Live Bot Hands Pairing Codes](./claude-plugins-official-live-bot-hands-pairing-codes/)
- [Claude Plugins Official Same Assistant Code Runs Discord](./claude-plugins-official-same-assistant-code-runs-discord/)
- [Knowledge Work Plugins Responding Webhook After Processing Silen](./knowledge-work-plugins-responding-webhook-after-processing-silen/)

<!-- END BRUTALIST REBUILD GUIDE -->
