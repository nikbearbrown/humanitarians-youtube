...
Merhaba — this is Kore, in for Humanitarians AI. You are working with Jira Cloud. The jira-api skill gives you two API families — Platform REST v3 for issues, projects, and JQL search; Agile REST v1 for boards and sprints — plus a JQL search script and a session helper. The philosophy: body text fields are a JSON tree called Atlassian Document Format, not plain text. Status moves through transitions, not direct sets…
