...
Konnichiwa — this is Kore, in for Humanitarians AI. Ask Claude to create an agent, add a subagent, write agent frontmatter, or get guidance on triggering conditions — the Agent Development skill fires. Agents are autonomous subprocesses that handle complex, multi-step tasks independently. They live in a markdown file with YAML frontmatter — name, description, model, color, tools — and a system prompt body. The descr…
