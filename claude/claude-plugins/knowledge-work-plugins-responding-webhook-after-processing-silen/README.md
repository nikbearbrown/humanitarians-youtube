...
# Why responding to a webhook after processing it silently doubles every event

## What this video is about

**Topic:** Webhook response ordering and the retry loop it controls

A handler receives `meeting.rtms_started`, writes to a database, then sends 200. The write takes 4s. The platform waited 3s, declared the endpoint unresponsive, and re-delivered. Two sessions open; two rows inserted.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/knowledge-work-plugins/partner-built/zoom-plugin/skills/rtms/references/quickstart.md`
