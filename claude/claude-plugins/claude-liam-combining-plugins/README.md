...
Sawubona — this is Liam, in for Bear. Install more than one plugin and you never have to say which. Claude reads the request and reaches for whatever fits. The real power was never one plugin. It's the stack.

The current plan contains **36 beats**. Its runtime is derived from 36 beats. The source recorded by the project is: books/anthropics/books/claude-cowork-plugins/chapters/ch13-combining-plugins.txt
