...
Bonjour — this is Liam, in for Bear. Ask Claude to pair a Discord user, approve someone, check who's allowed, or change the DM policy — the access skill for the Discord channel fires. This skill manages access control by editing a single JSON file: ~/.claude/channels/discord/access.json. It never talks to Discord directly — it just edits JSON and the channel server re-reads it. The security model is strict: this ski…
