...
**Topic:** ENTERPRISE SEARCH · CLAUDE TAG PLUGIN

Hej — this is Liam, in for Bear. Search the company's internal knowledge, ask what the policy is on something, find a prior decision, look up a project by name — the enterprise-search skill fires. It speaks the Glean Client REST API and works against any Glean-compatible backend. The workflow is three steps: search for ranked snippets, read the full documents that matter, and submit feedback on what you used. Two bu…
