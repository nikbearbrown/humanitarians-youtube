...
# Why enabling SMS breaks access control even when your number is allowlisted

## What this video is about

**Topic:** SMS spoofing as a bypass via unverified sender fields

`IMESSAGE_ALLOW_SMS=true`; `+15551234567` is allowlisted. An attacker uses an SMS gateway with `From: +15551234567`. The check finds the number and forwards to Claude — the attacker bypassed access control using a number they don't own.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-plugins-official/external_plugins/imessage/README.md`
