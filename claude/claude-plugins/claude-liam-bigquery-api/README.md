...
Namaste — this is Liam, in for Bear. Ask Claude to run SQL against BigQuery, check a job's status, list datasets, or preview table rows — the BigQuery API skill fires. It covers bigquery.googleapis.com/bigquery/v2 with a bundled script that handles both execution modes. The central concept is a job. Every query runs as a job in a billing project — the project that pays is not necessarily where the data lives. Two mo…
