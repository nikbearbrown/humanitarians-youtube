...
Merhaba — this is Liam, in for Bear. You are building a Claude plugin that reacts to events. The hook development skill gives you two hook types — Prompt-Based and Command — and two places to put them: plugin hooks.json or the user settings file. The philosophy: hooks let your plugin intercept the session lifecycle, but the mechanisms differ in where they run, when they trigger, and how they fail. Let me walk you th…
