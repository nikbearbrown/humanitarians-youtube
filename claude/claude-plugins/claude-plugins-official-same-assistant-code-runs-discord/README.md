...
# Why the same assistant code runs on Discord, Telegram, and iMessage unchanged

## What this video is about

**Topic:** MCP servers as platform adapters

Discord exposes `reply/react/edit/fetch_messages`; Telegram omits `fetch_messages` (no history); iMessage has `reply` and history but no reactions. Yet "reply with this" works identically on all three.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-plugins-official/external_plugins/telegram/README.md`
