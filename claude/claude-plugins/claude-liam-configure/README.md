...
Ciao — this is Liam, in for Bear. Paste a Discord bot token, ask to configure Discord, ask how to set this up, or ask who can reach you through the bot — and the configure skill fires. This skill has three modes: no arguments shows channel status and tells you your next step; a token argument saves the credential to disk; and the word clear removes the token. It writes to two state files and ends every setup interac…
