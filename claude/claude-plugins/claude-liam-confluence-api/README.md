...
Hola — this is Kore, in for Humanitarians AI. Ask Claude to find a Confluence page, read a doc, search the wiki, create or update a page, add a comment, list attachments, or paste an atlassian.net wiki URL — and the Confluence API skill fires. The skill covers two API generations that coexist in Confluence Cloud: REST v2 as the default for most operations, and REST v1 for CQL search, attachment upload, and label add…
