...
**Topic:** GOOGLE DRIVE API · CLAUDE PLUGINS

Merhaba — this is Liam, in for Bear. You are interacting with Google Drive through the Drive REST API v3. The skill gives you two base hosts, four things to know before the first request, and two bundled scripts that handle the hardest parts. The model: everything in Drive is a file — including folders. The trap: Workspace files have no bytes. Let me walk you through the architecture and the gaps.
