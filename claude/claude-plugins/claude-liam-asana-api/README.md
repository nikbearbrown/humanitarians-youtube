...
Hola — this is Kore, in for Humanitarians AI. Ask Claude to list your Asana tasks, complete a task, move one between sections, or comment on a project — the Asana API skill fires. It covers the Asana REST API at app.asana.com/api/1.0, with a bundled script for the most common workflows. Every resource in Asana has a gid — a string global ID — and every response wraps its payload in a top-level data key. Those two fa…
