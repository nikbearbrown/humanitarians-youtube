# BUILD LOG — claude-liam-troubleshooting
...
**Remotion patterns used (VR* series — all use CLAUDE tokens natively):**
  ...
  B01 (illustration/failure-to-abandonment) → SlateCard (unregistered custom illustration — will render as slate in previz; needs Remotion component or pantry still before final cut).
  ...
  B04 (illustration/reopen-reload) → SlateCard (unregistered custom illustration — will render as slate in previz; needs Remotion component or pantry still before final cut).
  C03 (SegmentCard) → VRSegmentCard — already uses CLAUDE tokens (cream/ink/terracotta); no additional retint needed.
  B09 (illustration/plugins-panel-toggle) → SlateCard (unregistered custom illustration — will render as slate in previz; needs Remotion component or pantry still before final cut).
