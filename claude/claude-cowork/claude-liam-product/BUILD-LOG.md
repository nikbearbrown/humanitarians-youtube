...
**Remotion patterns used (VR* series — all use CLAUDE tokens natively):**
  ...
  B04 (illustration/structured-thinking-partner) → SlateCard (unregistered custom illustration — will render as slate in previz; needs Remotion component or pantry still before final cut).
  ...
  B07 (SourceFlow) → VRSourceFlow — already uses CLAUDE tokens (cream/ink/terracotta); no additional retint needed.
  ...
  B19 (illustration/decision-doc-tradeoffs) → SlateCard (unregistered custom illustration — will render as slate in previz; needs Remotion component or pantry still before final cut).
  B20 (PredictCard) → VRPredictCard — already uses CLAUDE tokens (cream/ink/terracotta); no additional retint needed.
  ...
  B24 (SparkBeat) → VRSegmentCard — already uses CLAUDE tokens (cream/ink/terracotta); no additional retint needed.
