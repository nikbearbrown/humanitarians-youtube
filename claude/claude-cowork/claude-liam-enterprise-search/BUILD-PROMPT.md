# BUILD-PROMPT — Claude, Finding It (`claude-liam-enterprise-search`)
...
```
Build the deep-explainer reel at books/anthropics/books/claude-cowork-plugins/youtube/claude-liam-enterprise-search/.
...
2. LANE-MIX LINT. Run the beat-mix histogram over the 29 body beats. Expected:
   VOX 6 (20.7%), MANIM 8 (27.6%), REMOTION 9 (31.0%), CARD 6 (20.7%) — all in
   band. Confirm no >2 consecutive same-lane run except inside vox run R1 (B13→B14).
...
5. SHOPPING LIST (Gate D2, AFTER audio lock). For every VOX beat (B03, B13, B14,
   B17, B20, B23), first run python3 runtime/scripts/pantry_search.py on its
   ...
   are Tier 1 illustrative (no rights escalation, no real people/logos/legible
   names). B13→B14 is vox run R1 — honor the handoff blocks; B17 and B20 carry
   drawon terracotta annotations added at previz, not baked into the stills.
...
7. PANTRY FILL → REVIEW CUT. After I drop stills into pantry/, re-run (only changed
   slots recompile). Set shot.focus per still; verify the R1 handoff continuity
   (B13 email → B14 briefing, one continuous pull-back).
...
terracotta #D97757): scatter_across_platforms, filename_vs_content, graveyard_to_living,
grounded_vs_generic, prep_assemble_briefing, access_boundary, connect_and_index,
synthesis_across_docs. Remotion: PredictCard, ChipGrid, SourceFlow, code-block (Onda),
plus the illustration scenes named in the sheet (search-inside-document,
context-injection). Retint deck patterns to the claude stage (cream #F2F0E9,
...
VISUAL QC, IN-FOR-BEAR, GATE P. Never hardcode plugin counts, prices, or platform/
source lists — the source enumerates document connectors; keep them generic.
```
...
- **Render happens on your toolkit machine**, not in the authoring sandbox — this
  ...
  in place; the sandbox holds authoring artifacts only.
- **Who it's FOR is load-bearing.** The cold open waves off the intimidating name,
  then pitches the plugin at teams/orgs where knowledge is scattered across many
  tools and people. Keep that framing intact through B03 and the verdict — do not
  flatten it back to a solo-operator utility.
