...
# Why the "natural" trigger for a review-counting check is the one that lets the author bypass it

## What this video is about

**Topic:** GitHub event type → which branch supplies the workflow file

A PR author edits the workflow to `required_approvals: 0`. Under `pull_request_review`, GitHub runs the attacker's modified file from the merge ref and the gate passes with zero approvals; under `issue_comment`/`pull_request_target` it runs the base-branch file the author can't touch.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-code-action/agent-approval-check/README.md`
...
Recommended builder: **`cli-explainer`**. Use `ai-explainer` for one tight insight, `cli-explainer` when the prompt → real code → moving output → revision loop is the subject, and `deep-explainer` for a multi-act 5–10 minute documentary.

## Research prompt

Paste this into your research workflow before rewriting the video:

> Research **“Why the "natural" trigger for a review-counting check is the one that lets the author bypass it”** for an educational explainer about **GitHub event type → which branch supplies the workflow file**. Start from the source noted in this folder, then locate primary sources, official documentation, peer-reviewed research, standards, or original datasets. Identify the central question, one concrete key case, the mechanism that resolves it, important terminology, dated or version-sensitive claims, credible disagreements, limitations, and visual evidence suitable for animation. Return a claim table with: claim, exact source URL or citation, publication date, quoted or pinpoint evidence, confidence, and what must still be verified. Do not invent statistics, quotations, people, results, or historical details.
