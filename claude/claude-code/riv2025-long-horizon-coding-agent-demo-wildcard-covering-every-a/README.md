...
# Why a wildcard covering every Anthropic model still denies every cross-region call

## What this video is about

**Topic:** IAM resource-pattern prefix matching

The policy allows `foundation-model/anthropic.*`; the default model's ARN suffix is `us.anthropic.claude-opus-4-6-v1`. The wildcard requires the string to *start* with `anthropic.`, but this one starts with `us.` — every `InvokeModel` returns Access Denied.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/riv2025-long-horizon-coding-agent-demo/LEARNINGS.md`
