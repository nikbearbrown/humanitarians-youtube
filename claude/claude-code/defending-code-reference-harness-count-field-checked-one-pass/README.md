...
# Why a "count field" checked in one pass and used in another is a buffer overflow waiting between them

## What this video is about

**Topic:** Two-pass parsing desynchronization

A WAV `smpl` chunk parser validates `sampleLoopCount` in its count-prediction phase and allocates a 512-item heap buffer; the later read phase reads the same field from the stream again and loops that many times with no re-check. Attacker sends count=2000.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/defending-code-reference-harness/targets/drlibs/README.md`
