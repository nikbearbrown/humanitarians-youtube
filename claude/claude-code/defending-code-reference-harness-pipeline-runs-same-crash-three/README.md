...
# Why the pipeline runs the same crash three times before it believes it

## What this video is about

**Topic:** Reproducibility as a gate (the 3/3 rule)

A crafted WAV triggers a heap OOB write. The grader runs it three times; all three ASAN-abort identically → advances. A fuzzer artifact that crashes once due to malloc randomization → 1/3 → discarded.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/defending-code-reference-harness/docs/pipeline.md`
