...
**Topic:** WRITING HOOKIFY RULES · CLAUDE CODE SKILL

Ciao — this is Liam, in for Bear. Ask Claude to create a hookify rule, write a hook rule, configure hookify, or add a hookify rule — the Writing Hookify Rules skill fires. A hookify rule is a markdown file with YAML frontmatter that defines patterns to watch for and messages to show when those patterns match. Rules live in dot-claude slash hookify dot rule-name dot local dot md files, and are read dynamically on eve…
