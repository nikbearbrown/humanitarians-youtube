...
**Topic:** HOOK DEVELOPMENT · CLAUDE CODE SKILL

Bonjour — this is Liam, in for Bear. Ask Claude to create a hook, add a PreToolUse or Stop hook, validate tool use, block dangerous commands, or implement event-driven automation — the Hook Development skill fires. Hooks are event-driven automation scripts that execute in response to Claude Code events. Nine hook event types: PreToolUse, PostToolUse, UserPromptSubmit, Stop, SubagentStop, SessionStart, SessionEnd, Pr…
