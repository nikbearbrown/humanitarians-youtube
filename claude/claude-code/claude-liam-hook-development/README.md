...
Bonjour — this is Kore, in for Humanitarians AI. Ask Claude to create a hook, add a PreToolUse or Stop hook, validate tool use, block dangerous commands, or implement event-driven automation — the Hook Development skill fires. Hooks are event-driven automation scripts that execute in response to Claude Code events. Nine hook event types: PreToolUse, PostToolUse, UserPromptSubmit, Stop, SubagentStop, SessionStart, Se…
