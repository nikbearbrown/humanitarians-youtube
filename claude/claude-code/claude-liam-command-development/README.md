...
**Topic:** COMMAND DEVELOPMENT · CLAUDE CODE SKILL

Ciao — this is Liam, in for Bear. Ask Claude to create a slash command, add a command, write a custom command, define command arguments, or use CLAUDE_PLUGIN_ROOT — the Command Development skill fires. Commands are not agents. Commands are frequently-used prompts defined as Markdown files that Claude executes when invoked. Three locations: project commands in .claude/commands/, personal commands in ~/.claude/command…
