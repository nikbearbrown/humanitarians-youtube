# BUILD-PROMPT — Claude, Scouting (`claude-liam-research`)
...
```
Build the deep-explainer reel at books/anthropics/books/claude-cowork-plugins/youtube/claude-liam-research/.
...
2. LANE-MIX LINT. Run the beat-mix histogram. Target and authored actuals:
   VOX 23.1% (6), MANIM 26.9% (7), REMOTION 30.8% (8), CARD 19.2% (5) over 26 body
   beats — all inside band. Confirm no >2 consecutive same-lane run except the
   intended vox run B20→B21.

3. GATE P. Present the full narration (B00 → O01) on an animated SLATE PREVIZ for my
   sign-off. Do NOT generate audio until I approve. Channel is claude-liam:
   Kokoro af_kore, free. No ElevenLabs. Confirm the open ("Namaste — this is Kore,
   in for Humanitarians AI.") and sign-off ("Kore, in for Humanitarians AI.") land.
...
5. SHOPPING LIST (Gate D2, AFTER audio lock). For every VOX beat (B02, B08, B13,
   B16, B20, B21), first run python3 runtime/scripts/pantry_search.py on its
   ...
   are Tier 1 illustrative (no rights escalation, no real people, brands, or places).
   B20→B21 share one continuous room (vox_run R1) — shop them as a matched pair.
...
7. PANTRY FILL → REVIEW CUT. After I drop stills into pantry/, re-run (only changed
   slots recompile). Set shot.focus per still; author the B20→B21 handoff so the
   camera move is continuous across the matched desk.
...
terracotta #D97757): scale_day_to_hour, convergence_sources_to_meaning, positioning_map,
gap_coverage_grid, divergence_purpose_focuses, compare_two_options, contradictions_retained
— all MOVING DATA (a bar collapsing, dots converging, a 2-axis positioning map, a
coverage grid with a gap, a divergence spray, a weighted comparison, retained
contradictions). Remotion: ChipGrid, SourceFlow, PredictCard, code-block (Onda), plus
the illustration scenes named in the sheet (summary-vs-synthesis, citation-tracking,
market-entry-briefing). Retint deck patterns to the claude stage (cream #F2F0E9,
ink #3D3929, accent #D97757) and log the retint.

FACT-CHECK — enforce in narration AND on screen:
- Do NOT claim a dedicated official "Research plugin" SKU exists. Keep it capability-first
  (market-research workflows / research support). The title cards are branding only.
- Never hardcode plugin counts, prices, platform lists, or "as of [month]" framing.
- Reference managers / example companies are fleeting examples only, never a beat subject.
