...
**Topic:** The BOS token as an attention sink that corrupts aggregate metrics

For head 3, layer 4, scanning 50k sequences, the max-attention position is token 0 with weight > 0.55 in 91% of cases; the real verb→subject dependency only appears once position 0 is excluded from the max.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/headvis/README.md`
