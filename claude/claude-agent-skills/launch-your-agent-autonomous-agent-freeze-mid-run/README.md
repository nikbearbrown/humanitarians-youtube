...
# Why an autonomous agent can freeze mid-run and wait for a human indefinitely

## What this video is about

**Topic:** Session idle-bounce as a human-approval gate

An SRE agent diagnoses a latency spike, drafts a hotfix PR, and calls a custom `approve_pr()` tool. The session flips to `idle` with `stop_reason: requires_action`; the on-call engineer reviews and sends `tool_confirmation: allow`; the same session resumes and opens the PR.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/launch-your-agent/.claude/skills/launch-your-agent/references/cma-api.md`
