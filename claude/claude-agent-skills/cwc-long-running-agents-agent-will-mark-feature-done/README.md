...
# Why an agent will mark a feature "done" without proof — and how a hook makes proof structural

## What this video is about

**Topic:** Default-FAIL contract with evidence gating

The agent runs `npm test` (green) and tries to write `"passes": true` to `test-results.json`. A PreToolUse hook checks whether a screenshot or log was opened via Read this session; it wasn't, so the write is denied.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/cwc-long-running-agents/claude-code-config/.claude/hooks/verify-gate.sh`
