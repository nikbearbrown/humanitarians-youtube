...
# Why waiting for each result before the next click wastes time the agent knew it didn't need

## What this video is about

**Topic:** Batching predictable tool calls

"Open Terminal, type `ls`, press Enter." Sequential: 3 round trips × 5s = 15s, each waiting on the prior result. Batched via `computer_batch`: one request emits all three actions, the harness runs them locally, results return together — 5s total.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-quickstarts/computer-use-best-practices/README.md`
