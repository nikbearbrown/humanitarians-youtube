...
# Why sending the full list beats sending only what changed

## What this video is about

**Topic:** Level-triggered vs. edge-triggered state delivery

A consumer accumulates `task_started` and removes on completion; one `task_started` is dropped, so it permanently undercounts. `background_tasks_changed` replaces this by emitting the COMPLETE current set on every change.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/claude-agent-sdk-typescript/CHANGELOG.md`
