...
**Topic:** AGENT DEVELOPMENT · CLAUDE CODE SKILL

Hola — this is Liam, in for Bear. Ask Claude to create an agent, add a subagent, write agent frontmatter, or design triggering conditions for a Claude Code plugin — the Agent Development skill fires. Agents are not commands. Agents handle complex multi-step tasks autonomously; commands are user-initiated. An agent is a markdown file: YAML frontmatter with name, description, model, color, and optionally tools, plus a…
