...
# Why an agent's memory guardrail is one the model can't see

## What this video is about

**Topic:** Tools (model-visible) vs. hooks (invisible lifecycle callbacks)

Stage-3 memory uses both at once — a `save_memory` tool the model chooses to call, and a `UserPromptSubmit` hook that silently injects saved context before every turn without the model's knowledge.

The current plan contains **6 beats**. Its runtime is derived from 6 beats. The source recorded by the project is: `anthropics/agent-sdk-workshop/docs/CHEATSHEET.md`
