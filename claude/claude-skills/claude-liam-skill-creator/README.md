...
Bonjour — this is Liam, in for Bear. Ask Claude to create a skill from scratch, improve an existing one, or optimize how a skill triggers. The Skill Creator fires. And it is not a prompt-writing helper. It runs a full scientific build loop: capture intent, draft the SKILL.md, spawn real test cases in parallel with and without the skill, grade assertions quantitatively, show you the results in a built eval viewer, re…
