...
Konnichiwa — this is Liam, in for Bear. Any task where a spreadsheet file is the primary input or output — open a CSV, build a financial model, clean messy tabular data, add computed columns — the XLSX skill fires. The deliverable must be a spreadsheet file. Two tools: pandas for data analysis and bulk operations, openpyxl for formulas, formatting, and Excel-specific features. One mandatory step: after writing any f…
