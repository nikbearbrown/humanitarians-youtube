...
Merhaba — this is Kore, in for Humanitarians AI. You are building a model-invoked skill for Claude — the example-skill is the reference template. Unlike commands, which users invoke with a slash, skills are activated by Claude autonomously based on context. The description field is the trigger. The file lives at skills slash name slash SKILL.md. Let me walk you through the schema, the trigger mechanism, and the tear…
