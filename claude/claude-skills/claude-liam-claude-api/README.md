...
Bonjour — this is Liam, in for Bear. Ask Claude to add adaptive thinking to your Python app. Before Claude writes a single line, a skill fires — reads a reference, checks a drift table, confirms the exact model ID. That skill is called claude-api, and today we tear it down.

The current plan contains **9 beats**. Its runtime is derived from 9 beats. The source recorded by the project is: See the local source and fact-check files.
