...
Namaste — this is Liam, in for Bear. Ask Claude to build a complex dashboard, a multi-tab tool, or anything requiring state management, routing, or shadcn/ui components as a claude.ai artifact. The Web Artifacts Builder fires. It is not for simple single-file HTML. It provisions a full React 18 plus TypeScript plus Tailwind plus shadcn/ui stack in one init script, then bundles everything down to a single self-contai…
