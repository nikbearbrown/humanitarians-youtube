...
Hei — this is Liam, in for Bear. Ask Claude to build a landing page with a strong visual identity. Before it writes a line of CSS, the frontend-design skill fires. The first thing it says: there are three looks every AI defaults to, and this brief probably does not want any of them. The skill positions itself as a design lead, not a template factory. Let me show you what it requires.

The current plan contains **8 beats**. Its runtime is derived from 8 beats. The source recorded by the project is: See the local source and fact-check files.
