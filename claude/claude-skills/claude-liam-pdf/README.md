...
Hola — this is Liam, in for Bear. Hand Claude any PDF task: extract the tables, merge three documents, fill a form, OCR a scan. The PDF skill fires. It does not reach for one library. It routes: pypdf for manipulation, pdfplumber for extraction, reportlab for creation. Let me show you the routing logic and the one gotcha that will burn you if you miss it.
