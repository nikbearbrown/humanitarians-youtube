...
B03's equation frames were inspected in **both** output aspects, from the
rendered beat clips (`media/B03.mp4`, `vertical/media/B03.mp4`) and spot-checked
in the clean masters. Frames kept in `_qc/look/` and `vertical/_qc/look/`.

| Aspect | Frames inspected | Result |
|---|---|---|
| 16:9 (3840×2160) | 15% (2.96s), 50% (9.87s), 85% (16.77s) | **pass** |
| 9:16 (2160×3840) | 15%, 50%, 85%, full-res crop of the three rows at 85%, master at 60s | **pass** |

Reveal cues (measured mp3, 19.73s): row 1 at 2.02s ("Before resizing"), row 2 at
4.63s ("by the largest"), row 3 at 10.44s ("A Nikon frame").

- Staged reveal verified: row 1 alone at 15%, rows 1–2 at 50%, all three at 85%.
  No invalid intermediate equality is ever shown.
- Fraction bars on w/640 and h/640 render as real rules, numerator and
  denominator grouped, inside sized floor brackets that span the full fraction.
- The set-builder braces, `∈`, `≤`, `⌊ ⌋`, `⇒` all resolve to real STIX glyphs —
  no tofu, no fallback. `min`, `max` upright; r, f, d, w, h italic.
- Nothing clipped at any edge in either aspect.
- Portrait legibility: rows 2–3 have aspect ≈ 14, so they render at 126px =
  **3.3% of frame height** (full expression box) — above the 2% floor, crisp at
  full resolution, smaller than landscape by design of `TypesetMath`'s width cap.

**Observation, not a defect.** The `conditions` line renders at ~1.6% of frame
height in portrait (fixed 32·unit type in the component). It is fine print; the
numbers it carries are also spoken in the narration.

**Portrait-only copy fix.** The note's two dimensions were bound with no-break
spaces in `vertical/beat_sheet.json` so "1392 × 928" no longer splits across
lines in 9:16. Landscape copy unchanged.
