...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.21, 'copy': 'Chunking', 'sub': 'the quietest quality lever', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.52, 'copy': 'Step = size − overlap.', 'sub': 'that subtraction is the whole mechanism'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.43, 'copy': 'Fixed cuts blind.', 'sub': 'recursive stops at real boundaries'}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.77, 'copy': 'Same settings, more chunks.', 'sub': "recursive won't pack to the limit"}

class B05(BeatScene916):
    spec = {'kind': 'list', 'dur': 10.69, 'copy': '1,500 chunks, bimodal', 'items': ['short: headers & fragments', 'tall: full paragraphs', 'min 67 · max 388 · median 104']}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.7, 'copy': 'The isolation test.', 'sub': "if it can't stand alone, it's wrong"}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 4.78, 'copy': 'Cut on real boundaries.', 'sub': 'read one alone', 'handle': True}
