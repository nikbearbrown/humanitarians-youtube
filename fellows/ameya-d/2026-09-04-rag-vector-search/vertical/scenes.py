...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.3, 'copy': 'Vector Search at Scale', 'sub': 'brute force to HNSW & IVF', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.52, 'copy': 'Brute force dies at scale.', 'sub': 'compare every vector: 1,500 → 4ms, 10M → minutes'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.49, 'copy': "It's always one trade.", 'sub': 'recall vs latency vs memory'}

class B04(BeatScene916):
    spec = {'kind': 'list', 'dur': 8.36, 'copy': 'Three indexes', 'items': ['Flat — exact, under 100k', 'HNSW — a graph you walk', 'IVF — clusters you skip']}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.13, 'copy': 'HNSW: big hops, then small.', 'sub': '~O(log n), high recall, high RAM'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.0, 'copy': 'IVF: search a few clusters.', 'sub': 'nprobe tunes recall vs speed'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 5.72, 'copy': 'Flat until it hurts.', 'sub': 'then a graph or clusters', 'handle': True}
