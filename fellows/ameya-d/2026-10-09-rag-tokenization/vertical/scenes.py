...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.19, 'copy': 'Tokenization', 'sub': 'why financial text costs more', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.68, 'copy': 'Text becomes tokens.', 'sub': 'common word = 1; everything else shreds'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.73, 'copy': 'Same 20 characters.', 'sub': "'the cat sat on a mat' = 6 · '$4,829.17 EBITDA Q3x' = 13"}

class B04(BeatScene916):
    spec = {'kind': 'list', 'dur': 9.0, 'copy': 'Characters per token', 'items': ['prose ≈ 8.00', 'English ≈ 4.00', 'dense finance ≈ 1.67']}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.94, 'copy': '2–3× worse: three costs.', 'sub': 'cost · context · silent truncation'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.56, 'copy': 'mpnet truncates at 384.', 'sub': 'no error, no warning'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 6.49, 'copy': 'Count in tokens.', 'sub': 'not characters', 'handle': True}
