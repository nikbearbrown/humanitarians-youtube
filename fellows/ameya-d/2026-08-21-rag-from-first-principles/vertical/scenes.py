...
TAG = 'RAG SERIES · FIRST PRINCIPLES'
...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 10.03, 'copy': 'RAG From First Principles', 'sub': 'build it, then take it apart', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.3, 'copy': "A model can't answer from your docs.", 'sub': 'they were never in its training data'}

class B03(BeatScene916):
    spec = {'kind': 'list', 'dur': 7.94, 'copy': 'Four moves', 'items': ['chunk the text', 'embed to vectors', 'retrieve by cosine', 'ground the answer']}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.02, 'copy': 'Cosine, all at once.', 'sub': 'one matrix multiply scores every chunk'}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 6.91, 'copy': 'Watch the top-k bug.', 'sub': 'sort descending, or you return the worst matches'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 6.51, 'copy': "Measure. Don't assume.", 'sub': 'test retrieval before you trust the answer'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 5.31, 'copy': 'Measure. Do not assume.', 'sub': 'full cut on the channel', 'handle': True}
