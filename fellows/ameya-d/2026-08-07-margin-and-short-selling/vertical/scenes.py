...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 10.99, 'copy': 'Leverage Cuts Both Ways', 'sub': 'margin & short selling, plainly', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.96, 'copy': 'A 2x multiplier.', 'sub': '$10k cash + $10k margin controls $20k (Reg T 50%)'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.13, 'copy': 'It cuts down, too.', 'sub': "stock −20% → your equity −40%; the loan doesn't shrink"}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.94, 'copy': "You don't control the exit.", 'sub': 'below ~30% maintenance margin, the broker forces the sale'}

class B05(BeatScene916):
    spec = {'kind': 'list', 'dur': 11.86, 'copy': "Short selling's asymmetry", 'items': ['long: worst case is −100%', 'short: the loss is unbounded', 'a rising stock never has to stop']}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 6.85, 'copy': 'A multiplier, not a strategy.', 'sub': 'know the downside before you borrow'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 5.7, 'copy': 'Leverage cuts both ways.', 'sub': 'full cut on the channel', 'handle': True}
