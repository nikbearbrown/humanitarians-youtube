...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 8.81, 'copy': 'Quantization & Storage', 'sub': 'your vectors have a price tag', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.94, 'copy': 'Storage = n × dims × bytes.', 'sub': '1M × 768 × 4B ≈ 3 GB'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.09, 'copy': 'Dimensions cost twice.', 'sub': 'halve them → half storage AND half search'}

class B04(BeatScene916):
    spec = {'kind': 'list', 'dur': 8.79, 'copy': 'Shrink the numbers', 'items': ['int8 — 4× smaller, small loss', 'binary — 32× smaller, rescore', 'full — small index, spare RAM']}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.74, 'copy': 'Search cheap, then rescore.', 'sub': 'compressed shortlist → re-rank with full vectors'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.65, 'copy': '384-dim beat 768-dim.', 'sub': '88% vs 76% · half storage · 3× faster'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 5.33, 'copy': 'Dimension is a cost decision.', 'sub': 'better AND cheaper', 'handle': True}
