...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 9.54, 'copy': 'RAG: Answers That Matter', 'sub': 'the field standard, plainly', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.58, 'copy': 'Measure two halves.', 'sub': 'retrieval: hit@k, MRR · generation: faithfulness'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 6.14, 'copy': 'Debug retrieval first.', 'sub': 'bad context, and no prompt saves the answer'}

class B04(BeatScene916):
    spec = {'kind': 'list', 'dur': 8.94, 'copy': 'The judge has tells', 'items': ['favours the first option', 'favours longer answers', 'favours its own family']}

class B05(BeatScene916):
    spec = {'kind': 'list', 'dur': 9.43, 'copy': 'What moves quality most', 'items': ['chunking + hybrid retrieval', 'a cross-encoder reranks, never retrieves', 'validate the judge before you trust it']}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.96, 'copy': 'Validate the judge.', 'sub': 'a rubric, randomized order, checked against humans'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.81, 'copy': 'Measure what matters.', 'sub': 'full cut on the channel', 'handle': True}
