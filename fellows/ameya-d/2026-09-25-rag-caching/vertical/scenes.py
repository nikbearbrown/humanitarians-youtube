...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 8.49, 'copy': 'Caching for RAG', 'sub': 'the cheapest token you never generate', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.68, 'copy': 'A hit skips everything.', 'sub': 'cold ~1.3s · cache hit ~0.002ms'}

class B03(BeatScene916):
    spec = {'kind': 'list', 'dur': 9.05, 'copy': 'The layers', 'items': ['exact — the question string', 'semantic — catches paraphrases', 'prompt — ~90% off input']}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.24, 'copy': 'Semantic has a trap.', 'sub': 'too loose → the wrong answer, confidently'}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.17, 'copy': 'Invalidation is the hard part.', 'sub': 'doc changes → tag by source-id and purge'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.28, 'copy': 'Cache first.', 'sub': 'then route, then trim, then prompt-cache'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 6.17, 'copy': 'Skip the work.', 'sub': 'never the invalidation', 'handle': True}
