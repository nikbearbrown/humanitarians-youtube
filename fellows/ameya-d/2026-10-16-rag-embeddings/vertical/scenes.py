...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 6.93, 'copy': 'Embeddings & Batching', 'sub': 'the 12× win in one argument', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.11, 'copy': 'One vector per token.', 'sub': '(7,) → (7, 384) → mean pool → (384,)'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.06, 'copy': 'Then batch the multiply.', 'sub': '64 rows through one weight matrix, together'}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.49, 'copy': '12.6× faster.', 'sub': '64 separate: 14.9 ms · 1 batched: 1.18 ms'}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.03, 'copy': 'Identical arithmetic.', 'sub': '75,497,472 ops either way'}

class B06(BeatScene916):
    spec = {'kind': 'list', 'dur': 8.19, 'copy': 'At index scale', 'items': ['batch — biggest win', 'stream in shards', 'content-hash — only changes']}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.59, 'copy': 'Batch first.', 'sub': 'one oven, 1 cookie or 64', 'handle': True}
