...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.85, 'copy': 'LLM Inference', 'sub': 'why generation is the slow part', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.94, 'copy': 'One token at a time.', 'sub': '100 tokens = 100 forward passes, in order'}

class B03(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.11, 'copy': 'The KV cache saves you.', 'sub': 'reuse the past → O(n²) becomes O(n)'}

class B04(BeatScene916):
    spec = {'kind': 'list', 'dur': 9.0, 'copy': 'Make it faster', 'items': ['batch — throughput', 'quantize — cheaper hardware', 'speculative — fewer big steps']}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.26, 'copy': 'Stream the tokens.', 'sub': 'same total time, TTFT drops 2s → 200ms'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 7.51, 'copy': 'Fix the big number first.', 'sub': 'the model was 1288ms; vector search 0.2%'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 5.08, 'copy': 'The model is the clock.', 'sub': 'stream it, and it feels quick', 'handle': True}
