...
class B01(BeatScene916):
    spec = {'kind': 'title', 'dur': 9.96, 'copy': 'RAG in PyTorch', 'sub': 'answer questions from a PDF', 'handle': True}

class B02(BeatScene916):
    spec = {'kind': 'statement', 'dur': 9.26, 'copy': 'Ask a model about your PDF.', 'sub': "it says 'I don't know' — or invents an answer"}

class B03(BeatScene916):
    spec = {'kind': 'list', 'dur': 8.36, 'copy': 'Four moves', 'items': ['chunk the PDF', 'embed each chunk (384-dim)', 'retrieve the nearest', 'ground the answer']}

class B04(BeatScene916):
    spec = {'kind': 'statement', 'dur': 10.3, 'copy': 'Text becomes vectors.', 'sub': 'sentence-transformers on PyTorch, one row per chunk'}

class B05(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.92, 'copy': 'Search is one matrix multiply.', 'sub': 'q @ emb.T ranks every chunk by cosine — take the top 4'}

class B06(BeatScene916):
    spec = {'kind': 'statement', 'dur': 8.21, 'copy': 'The model reads only the top 4.', 'sub': 'grounded in your document, with a page to check'}

class B07(BeatScene916):
    spec = {'kind': 'title', 'dur': 7.83, 'copy': 'RAG, with the receipt.', 'sub': 'full cut on the channel', 'handle': True}
