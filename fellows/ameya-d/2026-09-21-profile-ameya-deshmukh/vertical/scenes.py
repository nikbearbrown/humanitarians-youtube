...
def _band(scene):
    ...
    tag = Text(TAG, color=INK, font_size=22, weight="BOLD").move_to(band.get_center())
    scene.play(GrowFromCenter(band), FadeIn(tag), run_time=0.6)
    return 0.6
...
class BeatScene916(Scene):
    ...
    def construct(self):
        ...
        spec = self.spec; target = float(spec["dur"]); elapsed = _band(self)
        ...
        if kind in ("title", "statement"):
            ...
            if sub:
                subt = _chip(sub, 22, 30, fill=INK, op=0.10, h=1.7).next_to(rule, DOWN, buff=0.55)
            ...
            chips.arrange(DOWN, buff=0.28).next_to(head, DOWN, buff=0.5)
...
class B01(BeatScene916):
    spec = {"kind": "title", "dur": 7.70, "copy": "Ameya Deshmukh", "sub": "AI / ML Engineer · a 90-second profile", "handle": True}

class B02(BeatScene916):
    spec = {"kind": "statement", "dur": 7.47, "copy": "Measure, don't guess.", "sub": "real numbers, before and after"}

class B03(BeatScene916):
    spec = {"kind": "statement", "dur": 10.99, "copy": "Production RAG, benchmarked.", "sub": "600 docs · retrieval MRR 0.66 → 0.81"}

class B04(BeatScene916):
    spec = {"kind": "list", "dur": 11.35, "copy": "More evidence",
            "items": ["Boehringer: 2M+ records, +40%", "LangChain multi-agent (health data)", "signal: significance + backtest"]}

class B05(BeatScene916):
    spec = {"kind": "statement", "dur": 7.81, "copy": "Ships it — and teaches it.", "sub": "a full RAG explainer series"}

class B06(Scene):
    def construct(self):
        self.camera.background_color = BG
        _band(self)
        panel = _panel(5.2, y=-0.35); self.play(FadeIn(panel), run_time=0.5)
        name = centered("Ameya Deshmukh", 15, color=INK, weight=BOLD, font_size=42).move_to([0, 1.5, 0])
        role = Text("AI / ML Engineer", color=ACCENT, font_size=26, weight="BOLD").next_to(name, DOWN, buff=0.3)
        self.play(Write(name), run_time=0.9); self.play(FadeIn(role), run_time=0.4)
        li_l = Text("LinkedIn", color=ACCENT, font_size=24, weight="BOLD").move_to([0, -0.3, 0])
        li = Text("linkedin.com/in/ameya-deshmukh-179945a4", color=INK, font_size=18, font="monospace").next_to(li_l, DOWN, buff=0.18)
        gh_l = Text("GitHub", color=ACCENT, font_size=24, weight="BOLD").next_to(li, DOWN, buff=0.5)
        gh = Text("github.com/Ameya-Deshmukh26", color=INK, font_size=18, font="monospace").next_to(gh_l, DOWN, buff=0.18)
        for m in (li_l, li, gh_l, gh):
            self.play(FadeIn(m, shift=UP * 0.1), run_time=0.4)
        self.play(FadeIn(Text("@HumanitariansAI", color=INK, font_size=24, weight="BOLD").move_to([0, -3.05, 0])), run_time=0.4)
        self.wait(max(0.4, 4.42 - self.renderer.time - 0.2))


class B07(BeatScene916):
    spec = {"kind": "title", "dur": 6.72, "copy": "An engineer who measures.", "sub": "@HumanitariansAI", "handle": True}
