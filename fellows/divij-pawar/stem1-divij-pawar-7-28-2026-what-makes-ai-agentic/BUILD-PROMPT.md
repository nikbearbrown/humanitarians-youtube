# BUILD-PROMPT.md — what-makes-ai-agentic
...
source script targets ~9 minutes; this narration measures **≈5.3 min**.
...
REEL="/c/Users/divij/Desktop/mycroft/accountability_layer/youtube/STEM1"
```
...
```bash
cd "$REEL"
declare -A S=( [B01]=B01_TierSpectrum [B02]=B02_TierZero [B03]=B03_TierOne \
               [B04]=B04_TierTwo [B05]=B05_TierThree [B06]=B06_TheChecklist )
...
```bash
mkdir -p _qc/frames
ffmpeg -i "$REEL/what-makes-ai-agentic.mp4" -vf fps=2 _qc/frames/%05d.png -y
```
...
Known-good from the pre-render pass (re-confirm after the real render): the
four-tier strip stays inside title-safe on B02–B06; B06's closing line is
centred under the bar, not anchored to the rightmost label.
