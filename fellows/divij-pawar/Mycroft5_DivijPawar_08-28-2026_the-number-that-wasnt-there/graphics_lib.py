...
def checked(text, size=26, color=None, weight=None, symbol="✓", buff=0.18,
            font=DISPLAY, trailing=False, **kw):
    ...
    Extra kwargs (e.g. line_spacing for multi-line words) pass through to
    the word's label(), never to the symbol's Text()."""
    sym = Text(symbol, font_size=_clamp(size), color=color)
    word = label(text, size=size, color=color, weight=weight, font=font, **kw)
