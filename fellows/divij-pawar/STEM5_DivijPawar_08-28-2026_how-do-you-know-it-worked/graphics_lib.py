...
# KERNING FIX: plain manim Text() renders these registered fonts with
# noticeably loose default letter-spacing (verified by direct comparison —
# Montserrat in particular reads distinctly airier than the same string set
# in any normal editor/browser, e.g. "Language" gapping between every
# letter). Pango's <span letter_spacing="..."> markup (via MarkupText)
# corrects this; plain Text has no equivalent knob in this manim version.
# Values are Pango units, tuned by rendering a comparison grid and picking
# the tightest value before glyphs start touching:
#   DISPLAY (Montserrat): -350  — plain is visibly loose; -600+ overlaps
#   SERIF (EB Garamond):  -100  — Garamond's own spacing is already fairly
#                                 tight; -300+ starts crowding
#   MONO (PT Mono):          0  — monospace fonts must keep uniform advance
#                                 widths for alignment; tightening this
#                                 defeats the point of using a mono face
TRACKING = {
    DISPLAY: -350,
    SERIF: -100,
    MONO: 0,
...
def _make_text(text, size, color, weight, font, **kw):
    """Builds a Text (or, when the font needs kerning correction, a
    MarkupText wrapped in a letter_spacing span) — see TRACKING above."""
    k = {"font_size": _clamp(size), "font": font, **kw}
    if color is not None:
        k["color"] = color
    if weight:
        k["weight"] = weight
    tracking = TRACKING.get(font, 0)
    if tracking:
        markup = f'<span letter_spacing="{tracking}">{_saxutils.escape(text)}</span>'
        return MarkupText(markup, **k)
    return Text(text, **k)


def label(text, size=28, color=None, weight=None, font=DISPLAY, **kw):
    """The default text unit — Montserrat, floor-clamped, kerning-corrected."""
    return _make_text(text, size, color, weight, font, **kw)
...
def mono(text, size=28, color=None, **kw):
    """Data / code / numbers only — never running prose. No kerning
    correction: tightening a monospace font's advance widths would break
    the column alignment that's the reason to use it."""
...
def checked(text, size=26, color=None, weight=None, symbol="✓", buff=0.18, font=DISPLAY):
    """Checkmark/X + word. Montserrat (and most display fonts) has no glyph
    for ✓/✕ — Pango silently falls back to a '.notdef' box showing the raw
    codepoint. The symbol renders in Manim's default font (which resolves
    it correctly); only the word takes the house DISPLAY font."""
    sym = Text(symbol, font_size=_clamp(size), color=color)
    word = label(text, size=size, color=color, weight=weight, font=font)
    return VGroup(sym, word).arrange(RIGHT, buff=buff)
