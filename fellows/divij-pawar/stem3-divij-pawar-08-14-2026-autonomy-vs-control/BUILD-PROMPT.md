# BUILD-PROMPT.md — autonomy-vs-control
...
source script targets ~9 minutes; this narration measures **≈5.2 min**.
...
REEL="/c/Users/divij/Desktop/mycroft/accountability_layer/youtube/STEM3"
```
...
```bash
cd "$REEL"
declare -A S=( [B01]=B01_TheBet [B02]=B02_BlastRadius [B03]=B03_ThreeModels 
               [B04]=B04_TheTradeoff [B05]=B05_SpeedAndVisibility [B06]=B06_TheQuestion )
...
```bash
mkdir -p _qc/frames
ffmpeg -i "$REEL/autonomy-vs-control.mp4" -vf fps=2 _qc/frames/%05d.png -y
```
...
Known-good from the pre-render pass (re-confirm after the real render): B04 carries NO axis numbers (the curves are qualitative by design —
see SOURCES.md); B05's three precedents are line marks, never photographs.
