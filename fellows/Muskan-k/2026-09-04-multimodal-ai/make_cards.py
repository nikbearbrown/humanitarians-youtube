...
def draw_flow(d, c, U):
    ...
    if not port:
        ix = 0.14 * W; iw = 0.17 * W; ih = 0.11 * H
        ys = [0.36, 0.48, 0.60, 0.72]
        hubx, huby, hubw, hubh = 0.50 * W, 0.55 * H, 0.24 * W, 0.20 * H
        for lab, yf in zip(inputs, ys):
            cy = yf * H
            chip(d, ix, cy, iw, ih, lab, U)
            arrow(d, ix + iw / 2, cy, hubx - hubw / 2 - 0.01 * W,
                  huby + (cy - huby) * 0.25, INK_SOFT, U["aw"], U["ah"])
        ...
        center(d, hubx, huby + hubh * 0.22, hub_sub, U["f_small"], INK_SOFT)
        ox = 0.85 * W
        arrow(d, hubx + hubw / 2, huby, ox - 0.085 * W, huby, SPARK, U["aw"], U["ah"])
        chip(d, ox, huby, 0.16 * W, 0.12 * H, out, U)
    else:
        # portrait: inputs 2x2 top, hub middle, output bottom
        iw = 0.40 * W; ih = 0.075 * H
        pos = [(0.29, 0.34), (0.71, 0.34), (0.29, 0.435), (0.71, 0.435)]
        for lab, (xf, yf) in zip(inputs, pos):
            chip(d, xf * W, yf * H, iw, ih, lab, U)
        hubx, huby, hubw, hubh = 0.5 * W, 0.60 * H, 0.62 * W, 0.11 * H
        for xf, yf in [(0.29, 0.435), (0.71, 0.435)]:
            arrow(d, xf * W, yf * H + ih / 2, hubx + (xf - 0.5) * 0.2 * W, huby - hubh / 2 - 0.008 * H,
                  INK_SOFT, U["aw"], U["ah"])
        rrect(d, [hubx - hubw / 2, huby - hubh / 2, hubx + hubw / 2, huby + hubh / 2],
              r=int(0.02 * W), fill=CARD, outline=SPARK, width=int(0.006 * W))
...
KINDS = {"flow": draw_flow, "grid": draw_grid, "example": draw_example, "versus": draw_versus}
