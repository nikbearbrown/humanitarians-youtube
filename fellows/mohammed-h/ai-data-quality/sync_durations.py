#!/usr/bin/env python3
"""sync_durations.py — push the measured mp3 length into each Dq* beat's props.
...
The Dq* compositions dodge that by deriving durationInFrames from a
...
whose pattern starts with `Dq`. After this runs, `useP()` inside each scene
...
def main() -> int:
    ...
    print(f"[sync] {len(touched)} Dq beat(s) pinned to their audio"
