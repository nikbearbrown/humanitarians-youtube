/**
 * ReelKit.tsx — the orientation-aware stage shared by the fellows/ritik-b reels.
 *
 * PROVENANCE: this file is the generalized half of `VqaKit.tsx` ("Visual Question
 * Answering, Blind?", 2026-09-06). The second reel in the series ("BLIP: Noisier
 * Data, Better Model?") needed the same stage, so the reusable primitives moved
 * here and VqaKit became a thin re-export shim. Nothing about the geometry changed.
 *
 * WHY NOT illustrations/kit.tsx: that kit is landscape-only — its SparkLine sits at
 * top:44, OUTSIDE the 9:16 title-safe inset of y=96, which trips GATE V's edge-bleed
 * BLOCKER on every portrait render. These reels ship in BOTH orientations from one
 * component set, so the stage has to be orientation-aware.
 *
 ...
 * in both orientations, and only the LAYOUT DIRECTION has to flip. Every scene
 * built on this kit keeps one set of font sizes and switches `flexDirection` on
 * `portrait`.
 *
 * COROLLARY, learned the hard way across 16 layout defects on the first reel: size
 * everything FROM `safe`, never from a number tuned by eye. A width that looks right
 * at 1728x972 is wrong at 972x1728 and fails the same way every time — content
 * clusters at the top and the long axis goes empty (or a content-sized flex row
...
/**
 * Stage — cream ground, SPARK-LINE LAW header, the illustration, and a footer that
 * carries the source (left) and the LOGO LAW channel bug (right). Every band lives
 * inside the title-safe box, so nothing here can trip GATE V's edge-bleed BLOCKER
 * in either orientation.
 *
 * `source` is the "no source, no verdict" slot: any beat that puts a published
 * number on screen passes its citation here and it renders under the artwork, in
 * the SAME FRAME as the number. That is the production gate these reels are held to.
 */
