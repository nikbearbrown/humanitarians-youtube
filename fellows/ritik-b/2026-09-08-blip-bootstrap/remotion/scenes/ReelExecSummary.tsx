...
import { Card, SANS, SERIF, Stage, clamp, ease, remap, useGeo, useP } from './ReelKit';

/**
 * ReelExecSummary — beat 2, the BLUF (EXECUTIVE-SUMMARY LAW).
 *
 ...
 */
export const reelExecSummarySchema = z.object({
...
export type ReelExecSummaryProps = z.infer<typeof reelExecSummarySchema>;

export const ReelExecSummary: React.FC<ReelExecSummaryProps> = ({
        ...
        <div style={{
          ...
          {roadmap.slice(0, 3).map((r, i) => (
            <Card key={r.label} style={{
              ...
              // centre in BOTH orientations: at 16:9 the cards are ~300px tall and
              // top-anchored content left a dead band under every one of them
              // (FILL-THE-CANVAS LAW, and GATE V's clustered MAJOR).
              justifyContent: 'center',
              padding: portrait ? '30px 32px' : '34px 30px 26px',
              transform: `translateY(${(1 - clamp(card(i), 0, 1)) * 18}px)`,
            }}>
              <div style={{ fontFamily: SERIF, fontSize: 58, fontWeight: 700, color: CLAUDE.SPARK, lineHeight: 1 }}>
                {i + 1}
              </div>
              <div style={{
                fontFamily: SANS, fontSize: 34, fontWeight: 700, color: CLAUDE.INK, marginTop: 14,
              }}>{r.label}</div>
              <div style={{
                fontFamily: SANS, fontSize: 31, color: CLAUDE.INK_SOFT, marginTop: 11, lineHeight: 1.34,
              }}>{r.body}</div>
            </Card>
          ))}
        </div>
